...
_term() {
	...
	HMLAND_PID=$(cat /var/run/hmland.pid)
	kill $HMLAND_PID
	wait "$HMLAND_PID"
	exit 0
}
...
if ! [ "$(ls -A /etc/homegear)" ]; then
	...
	echo "No config found, copy default config"
else
...
if ! [ "$(ls -A /var/lib/homegear)" ]; then
	...
	echo "No data found, copy default data"
else
	...
	[ $? -ne 0 ] && echo "Could not copy modules to \"homegear/modules/\". Please check the permissions on this directory and make sure it is writeable."
	...
	[ $? -ne 0 ] && echo "Could not copy nodes to \"homegear/node-blue/nodes\". Please check the permissions on this directory and make sure it is writeable."
	rm -Rf /var/lib/homegear/node-blue/node-red
	cp -a /var/lib/homegear.data/node-blue/node-red /var/lib/homegear/node-blue/
	[ $? -ne 0 ] && echo "Could not copy nodes to \"homegear/node-blue/node-red\". Please check the permissions on this directory and make sure it is writeable."

	rm -Rf /var/lib/homegear/node-blue/www
	cp -a /var/lib/homegear.data/node-blue/www /var/lib/homegear/node-blue/
	[ $? -ne 0 ] && echo "Could not copy Node-BLUE frontend to \"homegear/node-blue/www\". Please check the permissions on this directory and make sure it is writeable."
...
/etc/hmcfgusb.data/hmland -p 1234 -d -P
